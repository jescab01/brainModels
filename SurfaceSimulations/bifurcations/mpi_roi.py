...
coup_vals = np.arange(0, 10, 0.25)
taue_vals = np.arange(4, 40, 6)
taui_vals = np.arange(2, 40, 6)

params = [[g, taue, taui] for g in coup_vals for taue in taue_vals for taui in taui_vals]
...
local_results = roi_parallel(local_params)  # run the function for each parameter set and rank
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    fResults_df = pd.DataFrame(final_results,
                               columns=["vertex", "g", "tau_e", "tau_i",
                                        "maxPSP", "minPSP", "maxt1", "mint1",
                                        "maxt2", "mint2", "maxt3", "mint3"])

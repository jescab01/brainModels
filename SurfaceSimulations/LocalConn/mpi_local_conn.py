...
surf_types = ["disc8k"]
loc_conn_vals = np.linspace(0.001, 0.0075, 100)

params = [(surf, lc, 0) for surf in surf_types for lc in loc_conn_vals]
...
local_results = parallel_local_conn(local_params)  # run the function for each parameter set and rank
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    fResults_df = pd.DataFrame(final_results,
                               columns=["surf", "lc", "sigma", "g", "vertex",
                                        "maxSignal", "time_maxSignal", "intraS_std", "interS_std"])

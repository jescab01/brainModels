...
subjectid = ".2015MontbrioPazoRoxin"
wd = os.getcwd()
main_folder=wd+"\\"+subjectid
ctb_folder=wd+"\\CTB_data\\output\\"


emp_subj = "AVG_NEMOS_CB" # disconn-acc
...
m = models.DumontGutkin(I_e=np.array([10.0]), Delta_e=np.array([1.0]),
                        eta_e=np.array([-5.0]), tau_e=np.array([10.0]),
                        I_i=np.array([0.0]), Delta_i=np.array([1.0]),
                        eta_i=np.array([-5.0]), tau_i=np.array([10.0]),
                        tau_s=np.array([1.0]),
                        J_ee=np.array([0.0]), J_ei=np.array([15.0]), J_ie=np.array([15.0]), J_ii=np.array([10.0]),
                        Gamma=np.array([5.0]))
...
timeseriesPlot(raw_data, raw_time, regionLabels, main_folder)

# Fourier Analysis plot
FFTplot(raw_data, simLength, regionLabels, main_folder, max_hz=500)

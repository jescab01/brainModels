...
rois = [80, 93]  # Thalamus, Temporal_Inf_R  ::  weight = 0.012
...
# sigma_vals = np.arange(0, 0.05, 0.001)
sigma_vals = [0.01, 0.02, 0.04]
...
if spaces == "singles":
    ...
    params = [["p-sigmadiscrete_single", model, struct, subject, rois, r, 0, 15, 0.8, p, sigma] for model in models for r in range(n_rep) for p in p_vals for sigma in sigma_vals]
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    if len(df_results) == 16:
        df_results.columns = ["space", "model", "struct", "subject", "rois", "rep", "g", "speed", "w", "p", "sigma", "psp_t", "psp_dt", "fft", "simtime", "rPLV"]
    else:

...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\jesca\\OneDrive - Universidad Complutense de Madrid (UCM)\\Docencia\\PavloviaExps\\trial.py',
...
intrText = visual.TextStim(win=win, name='intrText',
    text='Elige "v" o "f".\n\nahora pulsa cualquier tecla para continuar',
    font='Arial',
...
instructionComponents = [intrText, key_resp]
...
thisExp.addData('intrText.started', intrText.tStartRefresh)
thisExp.addData('intrText.stopped', intrText.tStopRefresh)
...
trials = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('Sentences4Verification.xlsx'),
...
for thisTrial in trials:
    ...
    continueRoutine = Truej
    routineTimer.add(5.000000)
    # update component parameters for each repeat
    target.setColor('white', colorSpace='rgb')
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if target.status == STARTED:
            ...
            if tThisFlipGlobal > target.tStartRefresh + 4.5-frameTolerance:
        ...
        if response.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > response.tStartRefresh + 4.5-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if response.status == STARTED and not waitOnFlip:
            theseKeys = response.getKeys(keyList=['f', 'v'], waitRelease=False)

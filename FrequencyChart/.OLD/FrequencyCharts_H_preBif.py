...
for mode in ["fixed-prebif"]:
    Results = list()
    for He in np.arange(1.5, 8, 0.25):
        for Hi in np.arange(17, 30, 0.5):

            print(He)
            print(Hi)

            m.He = np.array([He, 3.25])
            m.Hi = np.array([Hi, 22])

            if mode == "balance-prebif":
                m.tau_e = np.array([32.5/He, 32.5/3.25])
                m.tau_i = np.array([440/Hi, 440/22])
            ...
            Results.append([mode, m.tau_e[0], m.tau_i[0], He,  Hi] +

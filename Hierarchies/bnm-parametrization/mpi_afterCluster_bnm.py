...
# simulations_tag = "PSEmpi_bnm-params-m06d05y2024-t15h.41m.28s"  # Classical
simulations_tag = "PSEmpi_bnm-params-m06d05y2024-t17h.45m.06s"  # Hierarchical
...
title = "%s-r426tract-FF%0.2fFB%0.2fL0" % (mode, ff, fb)


fig = make_subplots(rows=5, cols=5, y_title="Coupling factor (g)",
                    row_titles=["Frequency", "Power", "max-min", "intra_std", "inter_std"],
                    column_titles=[param for param, val in def_vals])

for j, (param, val) in enumerate(def_vals):

    sl = True if j == 0 else False

    ## He parameter :: column 1
    sub_v = [(p,v) for (p, v) in def_vals if (param, val) != (p, v)]

    sub = df.loc[(df["FF"]==ff) & (df["FB"]==fb) & (df[sub_v[1][0]]==sub_v[1][1]) & (df[sub_v[2][0]]==sub_v[2][1]) & (df[sub_v[3][0]]==sub_v[3][1]) &  (df[sub_v[0][0]]==sub_v[0][1])]

    fig.add_trace(go.Heatmap(z=sub["freq_peaks"], x=sub[param], y=sub.g, colorscale='Jet', showscale=sl,
                             zmin=0, zmax=max(df["freq_peaks"]),
                             colorbar=dict(thickness=10, len=0.15, y=0.9, title="Hz")),
                  row=1, col=j + 1)

    fig.add_trace(go.Heatmap(z=sub["band_modules"], x=sub[param], y=sub.g, colorscale='Viridis', showscale=sl,
                             zmin=0, zmax=max(df["band_modules"]),
                             colorbar=dict(thickness=10, len=0.15, y=0.70, title="dB")),
                  row=2, col=j + 1)

    fig.add_trace(go.Heatmap(z=sub["Smax"] - sub["Smin"], x=sub[param], y=sub.g, colorscale='Inferno', showscale=sl,
                             zmin=0, zmax= max(df["Smax"]-df["Smin"]),
                             colorbar=dict(thickness=10, len=0.15, y=0.5, title="mV")),
                  row=3, col=j + 1)

    fig.add_trace(go.Heatmap(z=sub["intraS_std"], x=sub[param], y=sub.g, colorscale='Inferno', showscale=sl,
                             zmin=0, zmax=max(df["intraS_std"]),
                             colorbar=dict(thickness=10, len=0.15, y=0.30, title="std")),
                  row=4, col=j + 1)

    fig.add_trace(go.Heatmap(z=sub["interS_std"], x=sub[param], y=sub.g, colorscale='Inferno', showscale=sl,
...
for j, (param, val) in enumerate(def_vals):
    ...
    fig.add_vline(x=val, row=1, col=j+1)

...
from toolbox import timeseriesPlot, FFTplot, FFTpeaks, PLV, epochingTool, paramSpace

# Define the name of the NMM to test
# and the connectome to use from the available subjects
subjectid = ".1995JansenRit"
emp_subj_list = ["subj"+str(i).zfill(2) for i in np.arange(2, 11, 1)]
...
# Jansen-Rit model with Stefanovski parameters
m = models.JansenRit(A=np.array([3.25]), B=np.array([22]), J=np.array([135]),
                     a=np.array([0.1]), a_1=np.array([1]), a_2=np.array([0.8]),
                     a_3=np.array([0.25]), a_4=np.array([0.25]), b=np.array([0.06]),
...
for emp_subj in emp_subj_list:
    ...
    coupling_vals = np.arange(5,6,1)
    speed_vals = np.arange(1,4,2)
    ...
    for g in coupling_vals:
        for s in speed_vals:
            ...
            for b in range(len(bands[0])):
                ...
                plv_emp=np.loadtxt(wd+"\\CTB_data\\output\\FC_"+emp_subj+"\\"+bands[0][b]+"plv.txt")
    ...
    ## GATHER RESULTS for each subject
    ...
    #["G", "speed", "plvD_r", "pliD_r", "aecD_r", "plvT_r","pliT_r", "aecT_r","plvA_r",
    ...
    paramSpace(dfPLV, 0.5, simname+"_PLV", folder=specific_folder)

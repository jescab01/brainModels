...
for g in coupling_vals:
    for s in speed_vals:
        ...
        for b in range(len(bands[0])):
            ...
            ## PLV
            plv = PLV(efPhase)
            # fname = "C:\\Users\\jesca\\PycharmProjects\\brainModels\\CTB_data\\output\\"+subjectid+"\\"+bands[0][b]+"plv.txt"
            # np.savetxt(fname, plv)

            # ## PLI
            pli = PLI(efPhase)
            # fname = "C:\\Users\\jesca\\PycharmProjects\\brainModels\\CTB_data\\output\\"+subjectid+"\\"+bands[0][b]+"pli"
            # np.savetxt(fname, pli)

            ## AEC
            aec = AEC(efEnvelope)
            # fname = "C:\\Users\\jesca\\PycharmProjects\\brainModels\\CTB_data\\output\\"+subjectid+"\\"+bands[0][b]+"corramp.txt"
            # np.savetxt(fname, aec)

            # Load empirical data to make simple comparisons
            plv_emp=np.loadtxt("C:\\Users\\jesca\\PycharmProjects\\brainModels\\CTB_data\\output\\FC_subj04\\"+bands[0][b]+"plv.txt")
            pli_emp=np.loadtxt("C:\\Users\\jesca\\PycharmProjects\\brainModels\\CTB_data\\output\\FC_subj04\\"+bands[0][b]+"pli.txt")
            aec_emp=np.loadtxt("C:\\Users\\jesca\\PycharmProjects\\brainModels\\CTB_data\\output\\FC_subj04\\"+bands[0][b]+"corramp.txt")

...
modes = ["jr", "jrd", "jr_pTh", "jrd_pTh"]

outputs = ["mV"]

coupling_vals = np.arange(0, 120, 1)
sigma_vals = np.arange(0, 0.1, 0.001)
n_rep = 1

params = [[subj, mode, g, s, r, out] for subj in subjects for mode in modes for g in coupling_vals for s in sigma_vals for r in range(n_rep) for out in outputs]
...
local_results = sigma_parallel(local_params)  # run the function for each parameter set and rank
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    fResults_df = pd.DataFrame(final_results, columns=["Subject", "Mode", "G", "sigma", "rep", "out", "IAF", "module", "bModule", "band", "rPLV"])

...
from jansen_rit_david_mine import JansenRitDavid2003, JansenRit1995
from mpi4py import MPI
import datetime

def sigma_parallel(params):
    result = list()
    ...
    for set in params:
        ...
        if "jrd" in mode:  # JANSEN-RIT-DAVID
            if "_def" in mode:
                ...
                sigma_array = np.asarray([s if 'Thal' in roi else 0 for roi in conn.region_labels])
            ...
            m = JansenRit1995(He=np.array([3.25]), Hi=np.array([22]), tau_e=np.array([1/0.1]),
                              tau_i=np.array([1/0.06]),
                              c=np.array([1]), c_pyr2exc=np.array([135]), c_exc2pyr=np.array([108]),
                              c_pyr2inh=np.array([33.75]), c_inh2pyr=np.array([33.75]),
                              p=np.array([0.1085]), sigma=np.array([sigma_array]),
        ...
        for b in range(len(bands[0])):
            ...
            ## Gather results
            result.append((emp_subj, mode, g, s, r, out, IAF[0], module[0], band_module[0], bands[0][b], plv_r))

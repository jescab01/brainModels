...
specific_folder = main_folder+"\\PSEsig"+subjectid+"-"+emp_subj+"-"+time.strftime("m%md%dy%Y-t%Hh.%Mm.%Ss")
    ...
    for s in speed_vals:
        ...
        for b in range(len(bands[0])):
            ...
            for i in range(n_perm):
                random_indexes = np.random.permutation(range(len(t1[0])))
                t1_temp[0, :] = t1[0, random_indexes]
                rand_distribution.append(np.corrcoef(t1_temp)[0, 1])

            newRow.append(scipy.stats.ttest_1samp(np.asarray(rand_distribution), plv_r).pvalue)
...
df = pd.DataFrame(results_fc, columns=["G", "speed", "plvD_r", "plvD_sig", "plvT_r","plvT_sig", "plvA_r", "plvA_sig", "plvB_r", "plvB_sig", "plvG_r", "plvG_sig"])


df.to_csv(specific_folder+"/PSEsig_FC"+simname+".csv", index=False)

df.columns = ["G", "speed", "Delta","Delta_sig", "Theta","Theta_sig", "Alpha","Alpha_sig", "Beta","Beta_sig", "Gamma","Gamma_sig"]


paramSpace(df, 0.5, emp_subj+subjectid+"_PLV", folder=specific_folder)
paramSpace(df, 0.01, "significance", folder=specific_folder)

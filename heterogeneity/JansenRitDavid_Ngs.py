...
m = JansenRitDavid2003_N(He1=np.array([3.25]), Hi1=np.array([22]),  # SLOW population
                         ...
                         p=np.array([0.1085]), sigma=np.array([0]))  # p! & sigma!
...
results = []
results_fft_peaks = []
for g in np.arange(0, 120, 1):
    for s in np.arange(0.5, 25, 1):
        ...
        results_fft_peaks.append((g, s, FFTpeaks(data, simLength-transient)[0][0],
...
df_fc = pd.DataFrame(results, columns=["G", "speed", "Delta", "Theta", "Alpha", "Beta", "Gamma"])
df_fft = pd.DataFrame(results_fft_peaks, columns=["g", "s", "peak", "module"])

paramSpace(df_fc, 0.5, emp_subj + subjectid + "_PLV")

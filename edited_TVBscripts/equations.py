...
class Sinusoid(TemporalApplicableEquation):
    ...
    def evaluate(self, var):
        ...
        _pattern = numpy.roll(_pattern, -numpy.int32(onset))
        _pattern[..., off] = 0.0

        return _pattern


class DriftSinusoid(TemporalApplicableEquation):
    """
    A Sinusoid changing its frequency in time.
    """

    equation = Final(
        label="Shifting Sinusoid Equation",
    ...
    parameters = Attr(
        field_type=dict,
        label="Sinusoid Parameters",
        default=lambda: {"amp": 1.0, "f_init": 10, "f_end":20, #freqs in Hz;
                         "onset": 100.0, "offset":400.0, # onset/offset in ms;
                         "feedback":False, "sim_length":1000, "dt":1}) #
    ...
    def evaluate(self, var):
        ...
        ################ DO IT
        f1, f2 = self.parameters["f_init"], self.parameters["f_end"]
        interval = (self.parameters["offset"] - self.parameters["onset"])/1000  # seconds in which the drift must be accomplished
        dt = self.parameters["dt"]/1000 #s
        n_steps = int(interval / dt)
        t = numpy.linspace(0, interval, n_steps)

        if self.parameters["feedback"] is False:
            f_inst=numpy.logspace(numpy.log10(f1), numpy.log10(f2), n_steps)
        elif self.parameters["feedback"] is True:
            f_inst0=numpy.logspace(numpy.log10(f1), numpy.log10(f2), int(n_steps/2))
            f_inst1=numpy.logspace(numpy.log10(f2), numpy.log10(f1), int(n_steps/2))
            f_inst=numpy.concatenate((f_inst0,f_inst1))

        phi=2*numpy.pi*numpy.cumsum(f_inst)*dt
        # plt.plot(t,numpy.sin(phi))

        fill_stim = numpy.concatenate((numpy.zeros((self.parameters["onset"])), numpy.sin(phi),
                                        numpy.zeros((self.parameters["sim_length"] - self.parameters["offset"]))))
        _pattern = fill_stim.reshape((1,len(fill_stim)))
        # plt.plot(var,_pattern[0])

...
class BaseHistory(StaticAttr):
    ...
    def __init__(self, weights, delays, cvars, n_mode, history):

        n_time = delays.max() + 1 if delays.max() + 1 > history.shape[0] else history.shape[0]

        self.n_time, self.n_cvar, self.n_node, self.n_mode = n_time, len(cvars), delays.shape[0], n_mode
    ...
    def from_simulator(cls, sim, initial_conditions=None):
        ...
        inst = cls(sim.connectivity.weights, sim.connectivity.idelays, sim.model.cvar, sim.model.number_of_modes, history)
...
class DenseHistory(BaseHistory):
    ...
    def __init__(self, weights, delays, cvars, n_mode, history):
        super(DenseHistory, self).__init__(weights, delays, cvars, n_mode, history)
...
class SparseHistory(DenseHistory):
    ...
    def __init__(self, weights, delays, cvars, n_mode, history):
        super(SparseHistory, self).__init__(weights, delays, cvars, n_mode, history)

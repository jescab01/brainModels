...
class SpatioTemporalPattern(SpatialPattern):
    ...
    def __call__(self, temporal_indices=None, spatial_indices=None):
        ...
        """
        pattern = None
        if self.temporal_pattern.shape[0] > 1 and temporal_indices is not None and spatial_indices is None:
            pattern = (self._spatial_pattern[:, 0] * self._temporal_pattern[:, temporal_indices])[:, numpy.newaxis]
        elif temporal_indices is not None and spatial_indices is None:
...
class MultiStimuli(StimuliRegion):
    ...
    def __init__(self, stimuli):
        super(MultiStimuli, self).__init__()
    ...
    def __call__(self, *args, **kwds):
        return numpy.array([stim(*args, **kwds) for stim in self.stimuli]).sum(axis=0)
...
class StimuliSurfaceRegion(SpatioTemporalPattern):
    """
    A spatio-temporal pattern defined in a Surface DataType.
    It includes the list of focal points.
    """
    ...
    focal_points_surface = NArray(dtype=int, label="Focal points triangles to stimulate", required=False)  # , locked=True, order=4)

    focal_regions = NArray(dtype=int, label="Regions to stimulate", required=False)  # , locked=True, order=4)
    ...
    def configure_space(self, region_mapping=None):
        ...
        MIXED Simulation (Surface and Regions) - Each stimulus is computed separately,
        surface through focal points and distance, and region thorugh weights.

        """
        dis_shp = (region_mapping.shape[0], numpy.size(self.focal_points_surface)  + 1)
        # rows(vertices + rois[unmmaped]), cols(n_focal_points+weights)

        distance = numpy.ones(dis_shp) * numpy.inf


        if self.focal_points_surface.any():
            k = -1
            for focal_point in self.focal_points_surface:
                k += 1
                foci = numpy.array([focal_point], dtype=numpy.int32)
                distance[:self.surface.number_of_vertices, k] = self.surface.geodesic_distance(foci)

        if self.focal_regions.any():
            for focal_region in self.focal_regions:
                distance[list(region_mapping).index(focal_region), -1] = 0

        super(StimuliSurfaceRegion, self).configure_space(distance)



class SpontaneousActivity(SpatioTemporalPattern):
    """
    A spatio-temporal pattern defined in a Surface DataType.
    It includes the list of focal points.
    """
    ...
    def configure_space(self, region_mapping=None):
        ...
        distance = numpy.ones((region_mapping.size, 1)) * numpy.inf

        # Corticals active; subcorticals inactive
        distance[:self.surface.number_of_vertices] = 0

        super(SpontaneousActivity, self).configure_space(distance)
    ...
    def configure_time(self, time):
        ...
        self.time = time
        # Generate a discrete representation of the temporal pattern.
        self._temporal_pattern = self.temporal.evaluate(self.time, self.spatial_pattern)
